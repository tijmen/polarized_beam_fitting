...
def generate_mock_data(config, n_sources=10):
    ...
    return (gaussfit_yoff, gaussfit_xoff, gaussfit_initial_amp, raw_maps, normalized_qu_templates, maps_numpy, maps_fft_numpy, source_ids, n_sources)
...
class TestFitterRecovery(unittest.TestCase):
    ...
    def test_gaussian_parameter_recovery(self, mock_load_data, mock_create_noise):
        ...
        fitter, best_fit_params, mock_data = self.run_recovery_test(mock_load_data, mock_create_noise, "gaussian")
    ...
    def test_bspline_profile_recovery(self, mock_load_data, mock_create_noise):
        ...
        fitter, best_fit_params, mock_data = self.run_recovery_test(mock_load_data, mock_create_noise, "b_spline")
        ...
        true_profile = beam_model.evaluate_beam_profile(beam_model.fit_gaussian_coefficients(true_fwhm), beam_model.r_fine_jax)
        fit_profile_T = beam_model.evaluate_beam_profile(best_fit_params["beams"][0]["T_coeffs"], beam_model.r_fine_jax)
